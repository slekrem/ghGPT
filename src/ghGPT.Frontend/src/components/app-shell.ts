...
export class AppShell extends LitElement {
  ...
  private onAccountChanged = (e: Event) => {
  ...
  render() {
    return html`
      <aside class="sidebar" @click=${this._onDocClick}>
        ...
        <div class="sidebar-footer" @click=${() => this.activeView = 'settings'}>
          ${this.account
            ? html`
  ...
  private renderView() {
    switch (this.activeView) {
      ...
      case 'settings':
        return html`<settings-view @account-changed=${this.onAccountChanged}></settings-view>`;
    }
  }
}
