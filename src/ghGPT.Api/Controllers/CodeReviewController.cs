...
public class CodeReviewController(ICodeReviewService codeReviewService) : ControllerBase
{
    [HttpPost("review")]
    public async Task StreamReview(string id, CancellationToken cancellationToken)
    {
        ...
        try
        {
            await foreach (var token in codeReviewService.StreamReviewAsync(id, cancellationToken))
            {
